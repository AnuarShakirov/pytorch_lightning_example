...
class DataPrepare:
    ...
    def __init__(self) -> None:
        ...
        self.stat_table: dict = {
            ...
            "DEPTH_TOP": [],
            "DEPTH_BOTTOM": [],
        }  # словарь для хранения статистики по данным
    ...
    def mask_data(
        ...
        for log in logs:
            mask = np.random.choice(  # noqa: NPY002
                [True, False],
                size=df_cur.shape[0],
                p=[mask_percentage, 1 - mask_percentage],
            )
    ...
    def split_by_slices(self) -> None:
        ...
        for well, df_well in pbar:  # группируем данные по скважинам
            for idx in range(
                ...
                len(df_well),
                SLICE_LEN,
            ):  # нарезаем данные на сегменты без перекрытия
                ...
                slice_data_torch: torch.Tensor = torch.as_tensor(
                    slice_data[LOGS].values,
                    dtype=torch.float32,
                )
                slice_data_masked_torch: torch.Tensor = torch.as_tensor(
                    slice_data_masked[LOGS].values,
                    dtype=torch.float32,
                )
                ...
                self.stat_table["slice_index"].append(cur_slice_index)
